...
上面这套是**默认拼法，不是标准答案**。Harness Kit 给的是一套有主见的默认值和一堆可单独开关的积木——**最好的 harness 是「长」出来的**，跟着你的项目、你的习惯和模型能力一起调形。每一项行为都在 `.harness/config.json` 里可配、可关，而且你**不必手写**——用自然语言让 Claude Code 改即可。

把调形也当成一个循环：

1. **先按默认值跑** —— `/harness-kit:init` 之后别急着调；用默认护栏做几轮真实任务，建立手感，看清楚哪里真的硌脚。
2. **观察** —— `/harness-kit:advisor` 看成熟度阶段与已解锁能力；`/harness-kit:trace-analyze` 看失败模式（哪些门常红、哪些文件反复改、会话是否失衡）。
3. **调形**（用自然语言告诉 Claude Code，它照 [配置指南](./docs/configuration.md) 改）：
   - **翻车就收紧** —— AI 又用新姿势出错？加一道门、一条分层规则，或一个 rubric 维度。
   - **碍事就放松** —— 某道门老挡正经活、循环阈值太敏感？关掉它、调高阈值，或把 `verificationMode` 降到 `advisory`。
   - **模型变强就精简** —— 新模型能原生处理的纪律就删掉；harness 里的假设会过时，留着只会拖慢你。
4. **按成熟度加码** —— 新项目从轻量起步（计划门 + 验证门足矣）；功能变多后再解锁更重的实践（独立评估 `evaluator`、strict 模式、自动派评估）。advisor 会提示你何时该升级。

> 没有「正确」的 harness，只有此刻适合这个项目的 harness。Harness Kit 给你积木和一套默认拼法，最终的工作流由你迭代成形，并随模型一起进化。
