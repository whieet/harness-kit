...
| 层 | 内容 | 调用 Claude？ | 何时跑 |
| --- | --- | --- | --- |
| **L1 单元** | smoke、d-套件（确定性/健壮性/并发） | 否 | push 到 main / PR（CI 三平台 × 两 Python 矩阵） |
...
CI 分工：Windows / macOS job 跑 smoke + structure + 回放；ubuntu job 跑全量确定性套件（含 d-套件）和 live e2e。本地任意平台 `python -m pytest tests` 都可全跑。
