...
2. **Relay** the phase and capabilities to the user. If the project just crossed into a new phase, point out the newly-available capabilities and suggest starting to use them (e.g. dispatching the `evaluator` subagent once you reach a multi-feature phase).

3. If no phase matches or metrics look wrong, the `metrics[]`/`phases[]` in `.harness/config.json` likely need tuning — offer to adjust them.
